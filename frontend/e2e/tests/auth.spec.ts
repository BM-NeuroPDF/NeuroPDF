...
test.describe('Authentication Flow', () => {
  test('user can login', async ({ page }) => {
    ...
    const statsText = page.locator('text=/İstatistiklerim|Genel Bakış|My Stats|Global Stats/i');
  ...
  test('user can logout', async ({ page }) => {
    ...
    const logoutButton = page.locator('button, a').filter({ hasText: /Logout|Çıkış/i });
    await expect(logoutButton.first()).not.toBeVisible({ timeout: 2000 }).catch(() => {
  ...
  test('user cannot login with invalid credentials', async ({ page }) => {
    ...
    const errorPopup = page.locator('.fixed.top-6.right-6').filter({ hasText: /CredentialsSignin|Invalid credentials|Geçersiz|Giriş yapılamadı/i });
