...
test.describe('Chat History E2E', () => {
  test.beforeEach(async ({ page }) => {
    await page.route('**/files/chat/sessions**', async (route) => {
      const req = route.request();
      const method = req.method();
      const path = new URL(req.url()).pathname;

      if (method === 'GET' && path === '/files/chat/sessions') {
        await route.fulfill({
          status: 200,
  ...
  test('lists mocked sessions on sidebar and restores chat on click', async ({
    ...
    await expect(
      page.getByRole('heading', { name: /Geçmiş sohbetler|Recent chats/i })
