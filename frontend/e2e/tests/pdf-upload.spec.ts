...
test.describe('PDF Upload E2E', () => {
  test('user can upload a PDF file', async ({ page }) => {
    ...
    await expect(page.locator('text=/uploadPageTitle|Neuro PDF|Yükleme|Upload/i').first()).toBeVisible({ timeout: 10000 });
    ...
    await expect(
      page.locator('[data-testid="pdf-viewer"]').or(page.locator('text=/sample\.pdf|currentFile/i')).first()
    ).toBeVisible({ timeout: 10000 });
    
    // Verify upload button is visible and enabled
    const uploadButton = page.locator('button').filter({ hasText: /uploadButton|Yükle|Upload/i }).first();
    ...
    await expect(
      page.locator('.fixed.top-6.right-6').filter({ hasText: /uploadSuccess|Yüklendi|Uploaded|success|Başarıyla/i }).first()
    ).toBeVisible({ timeout: 10000 });
  });
