...
vi.mock('react-markdown', () => {
  ...
  return {
    default: (props: any) => React.createElement('div', { 'data-testid': 'react-markdown-mock' }, props.children)
  }
})
...
vi.mock('next/dynamic', () => ({
  default: vi.fn((fn) => {
    const Component = ({ file, height }: any) => (
      <div data-testid="pdf-viewer">
        PDF Viewer: {file?.name || 'No file'}
      </div>
    )
...
describe('UploadPage Integration', () => {
  ...
  describe('File Drop / Upload Button', () => {
    ...
    it('should show error for invalid file type', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Geçersiz dosya türü|Invalid file type|Sadece PDF/i)).toBeInTheDocument()
      }, { timeout: 3000 })
    })
  ...
  describe('Upload Button Functionality', () => {
    it('should upload PDF to panel when upload button is clicked', async () => {
      ...
      const uploadButton = screen.getByRole('button', { name: /uploadButton|Yükle|Upload/i })
    ...
    it('should disable upload button while uploading', async () => {
      ...
      await waitFor(() => {
        const button = screen.getByRole('button', { name: /uploading|Yükleniyor/i })
    ...
    it('should show info message when upload button is clicked without file', async () => {
      ...
      const uploadButton = screen.queryByRole('button', { name: /uploadButton|Yükle|Upload/i })
  ...
  describe('Guest Limit Modal', () => {
    ...
    it('should show usage info when guest has remaining usage', () => {
      ...
      // Usage info should be displayed (not modal, just info box)
      const usageInfo = screen.queryByText(/remaining|kalan/i)
      // This might not always be visible depending on implementation
      // So we just check that limit modal is not shown
