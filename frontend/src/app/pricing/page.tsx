    ...
    <div className="min-h-screen pt-24 pb-12 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
          {/* 1. MİSAFİR KARTI */}
          <div
            className={`relative p-8 rounded-3xl border transition-all hover:shadow-xl bg-[var(--container-bg)] border-[var(--navbar-border)] ${userRole === 'Guest' ? 'ring-2 ring-gray-500 shadow-xl scale-[1.02]' : 'opacity-80 hover:opacity-100'}`}
          >
            {userRole === 'Guest' && (
            ...
            <p className="text-sm opacity-60 mb-8 h-10 text-[var(--foreground)]">
              {t('guestDesc') || 'Hesap açmadan hızlı işlemler.'}
            </p>

            <button
              disabled
              className="w-full py-3 rounded-xl font-semibold border border-gray-300 dark:border-gray-700 text-gray-500 cursor-default"
            >
            ...
            <ul className="mt-8 space-y-4">
              {features.map((f, i) => (
                <FeatureItem
                  key={i}
                  label={t(f.key as keyof typeof translations.tr) || f.key}
                  value={f.guest}
                />
              ))}
            </ul>
          ...
          {/* 2. STANDART KARTI */}
          <div
            className={`relative p-8 rounded-3xl border transition-all hover:shadow-2xl bg-[var(--container-bg)] border-blue-500/30 ${userRole === 'Standart' || userRole === 'Standard' ? 'ring-2 ring-blue-500 shadow-blue-500/20 shadow-2xl scale-[1.02] z-10' : ''}`}
          >
            ...
            <h3 className="text-xl font-bold mb-2 text-blue-500">
              {t('planStandard') || 'Standart'}
            </h3>
            <div className="text-4xl font-extrabold mb-6 text-[var(--foreground)]">
              {t('free') || 'Ücretsiz'}
            </div>
            <p className="text-sm opacity-60 mb-8 h-10 text-[var(--foreground)]">
              {t('standardDesc') || 'Üye olanlar için genişletilmiş limitler.'}
            </p>

            {userRole === 'Guest' ? (
              <button
                onClick={() => router.push('/login')}
                className="w-full py-3 rounded-xl font-semibold bg-[var(--button-bg)] text-white hover:brightness-110 transition-all shadow-lg"
              >
              ...
              <button
                disabled
                className="w-full py-3 rounded-xl font-semibold bg-blue-500/20 text-blue-500 cursor-default border border-blue-500/50"
              >
                {t('active') || 'Aktif'}
              </button>
            ) : (
              <button
                disabled
                className="w-full py-3 rounded-xl font-semibold border border-gray-600 text-gray-500 cursor-default"
              >
                {t('included') || 'Dahil'}
              </button>
            )}
          ...
          {/* 3. PRO KARTI */}
          <div
            className={`relative p-8 rounded-3xl border transition-all hover:shadow-2xl transform bg-gradient-to-b from-[var(--container-bg)] to-[var(--background)] border-purple-500/50 ${userRole === 'Pro' || userRole === 'Admin' ? 'ring-2 ring-purple-500 shadow-purple-500/30 shadow-2xl scale-[1.02] z-10' : ''}`}
          >
            ...
            <div className="text-4xl font-extrabold mb-6 text-[var(--foreground)]">
              ₺99<span className="text-lg font-normal opacity-50">/mo</span>
            </div>
            <p className="text-sm opacity-60 mb-8 h-10 text-[var(--foreground)]">
              {t('proDesc') || 'Profesyoneller için sınırsız AI gücü.'}
            </p>

            {userRole === 'Pro' || userRole === 'Admin' ? (
              <button
                disabled
                className="w-full py-3 rounded-xl font-semibold bg-purple-500/20 text-purple-400 cursor-default border border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.2)]"
              >
                {t('active') || 'Aktif'}
              </button>
            ) : (
...
function FeatureItem({
  ...
  return (
    <li className="flex items-center justify-between text-sm group">
      <span
        className={`font-medium text-[var(--foreground)] ${isPro ? 'opacity-100' : 'opacity-70'}`}
      >
        {label}
      </span>
      <span className="font-bold flex items-center gap-1">
        {value === true ? (
          <div
            className={`p-1 rounded-full ${isPro ? 'bg-purple-500/20 text-purple-400' : 'bg-green-500/20 text-green-500'}`}
          >
            <svg
          ...
          <span
            className={`${isPro ? 'text-purple-400' : highlight ? 'text-blue-500' : 'opacity-70 text-[var(--foreground)]'}`}
          >
            {value}
          </span>
        )}
