...
/** Sayfa numarasını [1, numPages] aralığına sıkıştırır; numPages henüz 0 iken üst sınır 1 kabul edilir. */
export function clampPdfPage(newPage: number, numPages: number): number {
...
export default function PdfViewer({ file, height = 700 }: Props) {
  ...
  const changePage = (newPage: number) => {
    setPage(clampPdfPage(newPage, numPages));
  };

  const buttonClass =
    'px-3 py-1.5 rounded-lg text-sm font-bold shadow-md transition-transform hover:scale-105 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed';
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3">
      ...
      <div
        className="rounded-xl border overflow-auto flex justify-center bg-gray-100 dark:bg-gray-900 flex-1 min-h-0"
        style={{
          borderColor: 'var(--container-border)',
        }}
      >
