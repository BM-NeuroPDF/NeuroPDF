...
export default function ClientPdfPanel() {
  ...
  // Panelin gizleneceği sayfalar
  const hiddenPaths = ["/", "/login", "/register", "/profile"];

  
  // Sağ panel dropzone — PDF bırakınca panele ekler
  ...
  // Panel bu sayfalarda gizlenir: hook'lar çağrıldıktan sonra güvenle çık
  if (hiddenPaths.includes(pathname)) return null;

  // PDF yoksa paneli gösterme
  ...
  return (
    <aside
      ...
      className={`w-80 h-full flex flex-col border-l transition-all duration-300 shadow-xl z-20
        ${isDragActive ? "bg-red-50 border-red-500" : ""}
      `}
      style={{
        ...
        <div
          ...
          onDragStart={(e) => {
            try {
              ...
              // Paneldeki aktif PDF'i gerçek bir File olarak sürükleme verisine ekle
              // Böylece upload dropzone masaüstünden sürüklenen dosyayla aynı şekilde alır
              if (pdfFile) {
                const file = new File([pdfFile], pdfFile.name || "document.pdf", {
                ...
                } else if ((e.dataTransfer as any).setDragImage) {
                  // Yedek olarak sadece görsel efekt kalsın; drop tarafında özel protokol eklenebilir
                }

                // Panel-origin işaretleyici; unified drop kancası bu veriyi yakalar
                try {
                  const meta = {
                    name: file.name,
              ...
              // Sürükleme verisi eklenemese bile kullanıcı akışı bozulmasın
              // Gerekirse ileride özel dataTransfer.setData protokolü eklenebilir
            }
          }}
          onDragEnd={(e) => {
