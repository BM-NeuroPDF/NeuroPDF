...
import { fetchStoredPdfBlob, swrFetcher, type UserDocumentListItem } from '@/utils/api';
import { usePdfActions, usePdfUi } from '@/context/PdfContext';
...
const PdfPreviewModal = dynamic(() => import('@/components/PdfPreviewModal'), {
  ...
  loading: () => (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="container-card rounded-2xl border p-4 text-sm">Önizleme yükleniyor...</div>
    </div>
  ),
});
...
function DocumentsClientPanel() {
  const router = useRouter();
  const { status } = useSession();
  const { saveExistingPdf } = usePdfActions();
  ...
  const queryError =
    error instanceof Error ? error.message : error ? 'Belgeler yüklenirken bir hata oluştu.' : null;
  const errorMessage =
  ...
  const openDocument = async (doc: UserDocumentListItem, target: 'edit' | 'chat') => {
    setOpeningId(doc.id);
    try {
      setActionError(null);
      ...
      const message =
        err instanceof Error ? err.message : 'Belge açılamadı. Lütfen tekrar deneyin.';
  ...
  const handleOpenPreview = async (doc: UserDocumentListItem) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Önizleme yüklenemedi. Lütfen tekrar deneyin.';
      setActionError(message);
    } finally {
    ...
    <section className="max-w-7xl mx-auto space-y-6">
      <div className="docs-surface p-6 rounded-3xl shadow-sm">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-widest opacity-60">My Documents</p>
            <h1 className="text-2xl sm:text-3xl font-bold">Belgelerim</h1>
            <p className="text-sm opacity-70 mt-1">
              Hesabınızda kayıtlı PDF belgelerinizi görüntüleyin ve kaldığınız yerden devam edin.
            </p>
          </div>
          <button
            type="button"
      ...
      {errorMessage && (
        <div className="error-box rounded-xl p-4 text-sm font-medium">{errorMessage}</div>
      )}
      ...
      {!loading && !hasDocuments && !errorMessage && (
        <div className="container-card rounded-3xl border p-10 text-center">
          ...
          <p className="text-sm opacity-70 mt-2">
            İlk PDF belgenizi yükleyerek bu alanda tüm geçmiş dokumanlarınızı görebilirsiniz.
          </p>
          <button
            type="button"
...
export default memo(DocumentsClientPanel);
