...
class TestAnalyzeTextWithLocalLLM:
    ...
    def _force_ollama_client_path(self, monkeypatch):
        """OpenAI uyumlu HTTP dalını kapat; mevcut ollama.Client mock testleri korunur."""
        monkeypatch.setattr(
    ...
    def test_analyze_text_summarize_success(self, mock_client_class):
        ...
        result = analyze_text_with_local_llm(
            text="Test text with errors", task="summarize", instruction="Summarize this"
        )
    ...
    @patch("app.services.local_llm_service.ollama.Client")
    def test_analyze_text_summarize_no_corrections(self, mock_client_class):
        ...
        result = analyze_text_with_local_llm(text="Original text", task="summarize")

        assert "summary" in result
        assert result["summary"] == "Summary"

    @patch("app.services.local_llm_service.ollama.Client")
    def test_analyze_text_summarize_invalid_json(self, mock_client_class):
        ...
        result = analyze_text_with_local_llm(text="Test text", task="summarize")
    ...
    def test_analyze_text_chat_success(self, mock_client_class):
        ...
        result = analyze_text_with_local_llm(
            text="User message", task="chat", instruction="Be helpful"
        )
    ...
    def test_analyze_text_chat_default_instruction(self, mock_client_class):
        ...
        result = analyze_text_with_local_llm(text="Message", task="chat")
    ...
    @patch("app.services.local_llm_service.ollama.Client")
    def test_analyze_text_summarize_exception_during_detection(self, mock_client_class):
        ...
        result = analyze_text_with_local_llm(text="Test", task="summarize")

        # Should still work
        assert "summary" in result

    @patch("app.services.local_llm_service.ollama.Client")
    def test_analyze_text_summarize_timeout(self, mock_client_class):
        ...
        assert "summary" in result
        assert (
            "hata" in result["summary"].lower() or "error" in result["summary"].lower()
        )
