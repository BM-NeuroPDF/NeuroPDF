...
class TestGeneralChat:
    ...
    def test_general_chat_local(self, mock_local_llm):
        ...
        answer, actions = general_chat(
            history_text="Previous messages", user_message="Hello", llm_provider="local"
        )
    ...
    def test_general_chat_local_no_answer(self, mock_local_llm):
        ...
        answer, _ = general_chat(
            history_text="", user_message="Test", llm_provider="local"
        )

        assert answer == "Local LLM yanıt üretmedi."

    @patch("app.services.llm_manager.ai_service.gemini_generate")
    def test_general_chat_cloud_language_en(self, mock_gemini):
        """general_chat English system_instruction branch (language=='en')."""
        mock_gemini.return_value = "Hello in English"
        ...
        assert answer == "Hello in English"
        assert actions == []
        call_kw = mock_gemini.call_args[1]
        assert "NeuroPDF's AI assistant" in call_kw["text_content"]
...
class TestBuildChatPrompt:
    ...
    def test_build_chat_prompt_language_en(self):
        """English system instruction branch in _build_chat_prompt."""
        result = _build_chat_prompt(
            pdf_context="Body",
            filename="doc.pdf",
            history_text="H1",
        ...
        assert "You are a PDF assistant" in result
