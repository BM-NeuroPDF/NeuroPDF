...
class TestLLMChoiceModel:
    ...
    def test_create_llm_choice(self, db_session, clean_db):
        """Test creating LLMChoice."""
        llm_choice = LLMChoice(id=10, name="test_llm")
        db_session.add(llm_choice)
        db_session.commit()

        assert llm_choice.id == 10
        assert llm_choice.name == "test_llm"

    def test_read_llm_choice(self, db_session, clean_db):
        """Test reading LLMChoice."""
        llm_choice = LLMChoice(id=11, name="read_test")
        db_session.add(llm_choice)
        db_session.commit()

        found = db_session.query(LLMChoice).filter(LLMChoice.id == 11).first()
        assert found is not None
        assert found.name == "read_test"

    def test_unique_constraint_llm_choice(self, db_session, clean_db):
...
class TestUserRoleModel:
    ...
    def test_create_user_role(self, db_session, clean_db):
    ...
    def test_unique_constraint_user_role(self, db_session, clean_db):
...
class TestUserModel:
    ...
    def test_create_user(self, db_session, clean_db):
        """Test creating User."""
    ...
    def test_read_user(self, db_session, clean_db):
        """Test reading User."""
        user_id = str(uuid.uuid4())
    ...
    def test_update_user(self, db_session, clean_db):
        """Test updating User."""
        user_id = str(uuid.uuid4())
    ...
    def test_delete_user(self, db_session, clean_db):
        """Test deleting User."""
        user_id = str(uuid.uuid4())
    ...
    def test_user_foreign_key_llm_choice(self, db_session, clean_db):
    ...
    def test_user_foreign_key_role(self, db_session, clean_db):
    ...
    def test_user_relationship_llm_choice(self, db_session, clean_db):
    ...
    def test_user_relationship_role(self, db_session, clean_db):
...
class TestUserAuthModel:
    ...
    def test_create_user_auth(self, db_session, clean_db, test_user):
    ...
    def test_user_auth_relationship_user(self, db_session, clean_db, test_user):
    ...
    def test_user_auth_cascade_delete(self, db_session, clean_db, test_user):
        ...
        found = db_session.query(UserAuth).filter(UserAuth.id == auth.id).first()
        assert found is None
...
class TestUserSettingsModel:
    ...
    def test_create_user_settings(self, db_session, clean_db, test_user):
    ...
    def test_user_settings_relationship_user(self, db_session, clean_db, test_user):
...
class TestUserStatsModel:
    ...
    def test_create_user_stats(self, db_session, clean_db, test_user):
    ...
    def test_user_stats_default_values(self, db_session, clean_db, test_user):
...
class TestPDFModel:
    ...
    def test_create_pdf(self, db_session, clean_db, test_user):
        """Test creating PDF."""
    ...
    def test_pdf_relationship_user(self, db_session, clean_db, test_user):
    ...
    def test_pdf_cascade_delete(self, db_session, clean_db, test_user):
        """Test cascade delete when User is deleted."""
        pdf_id = str(uuid.uuid4())
        pdf = PDF(
            id=pdf_id,
        ...
        found = db_session.query(PDF).filter(PDF.id == pdf_id).first()
        assert found is None
...
class TestGuestSessionModel:
    ...
    def test_update_guest_session(self, db_session, clean_db):
