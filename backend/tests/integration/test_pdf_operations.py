...
from app.storage import save_pdf_to_db, get_pdf_from_db, delete_pdf_from_db, list_user_pdfs
...
class TestPDFOperations:
    ...
    def test_save_pdf_to_db(self, test_db_session, clean_db, test_user):
        """Test saving PDF to database."""
        pdf_bytes = b"fake_pdf_content"
        filename = "test_document.pdf"
        
        pdf = save_pdf_to_db(test_db_session, test_user.id, pdf_bytes, filename)
    ...
    def test_get_pdf_from_db(self, test_db_session, clean_db, test_user):
        ...
        retrieved = get_pdf_from_db(test_db_session, pdf.id, test_user.id)
        
        assert retrieved is not None
        assert retrieved.id == pdf.id
        assert retrieved.pdf_data == pdf_bytes
    
    def test_get_pdf_wrong_user(self, test_db_session, clean_db, test_user):
    ...
    def test_delete_pdf_from_db(self, test_db_session, clean_db, test_user):
        ...
        result = delete_pdf_from_db(test_db_session, pdf.id, test_user.id)
        
        assert result is True
        assert get_pdf_from_db(test_db_session, pdf.id) is None
    
    def test_list_user_pdfs(self, test_db_session, clean_db, test_user):
        ...
        pdfs = list_user_pdfs(test_db_session, test_user.id)
        
        assert len(pdfs) == 3
        assert all(pdf.user_id == test_user.id for pdf in pdfs)
    
    def test_pdf_cascade_delete_with_user(self, test_db_session, clean_db, test_user):
