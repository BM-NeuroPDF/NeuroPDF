...
class TestUploadAvatar:
    ...
    def test_upload_avatar_success(
        self, mock_save_avatar, mock_upload, override_dependencies, mock_db, sample_png
    ):
    ...
    def test_upload_avatar_invalid_type(self, override_dependencies, sample_png):
...
class TestEditAvatar:
    ...
    def test_edit_avatar_success(
        ...
        response = client.post("/api/v1/user/me/avatar/edit", files=files, data=data)
...
class TestConfirmAvatar:
    ...
    @patch("app.routers.user_avatar_routes.get_temp_avatar")
    def test_confirm_avatar_not_found(self, mock_get_temp, override_dependencies):
