...
from app.routers.files import _legacy, routes_chat, routes_summarize
...
def test_chat_message_stream_proxies_sse(monkeypatch):
    ...
    monkeypatch.setattr(routes_chat.httpx, "AsyncClient", _FakeAsyncClient)
...
def test_summarize_stream_proxies_sse(monkeypatch):
    ...
    monkeypatch.setattr(
        _legacy, "get_user_llm_choice", lambda db, user_id: (1, "cloud")
    )
    client = TestClient(app)
    try:
