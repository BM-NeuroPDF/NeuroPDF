...
class StorageService:
    ...
    def __init__(self):
        ...
        Path.mkdir(self.base_dir, exist_ok=True, parents=True)
    ...
    def generate_file_path(user_id: Optional[str], filename: str) -> str:
        ...
        return (
            f"{user_id}/{file_id}_{safe_filename}"
            if user_id
            else f"guest/{file_id}_{safe_filename}"
        )
    ...
    async def upload_file(
        ...
        try:
            ...
            relative_path = self.generate_file_path(
                user_id, file.filename or "document.pdf"
            )
...
def save_avatar_to_db(
    db: Session, user_id: str, image_bytes: bytes, is_ai: bool = False
):
...
def get_pdf_from_db(
    db: Session, pdf_id: str, user_id: Optional[str] = None
) -> Optional[PDF]:
