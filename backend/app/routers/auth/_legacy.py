...
def _set_refresh_cookie(response: Response, refresh_token: str) -> None:
    response.set_cookie(
        ...
        max_age=max(60, settings.REFRESH_TOKEN_EXPIRES_DAYS * 24 * 3600),
        path="/",
    )
...
def google_login(
    ...
    if not check_rate_limit(
        request, "auth:google", settings.RATE_LIMIT_AUTH_PER_MINUTE, 60, "critical"
    ):
...
def register_user(
    ...
    if not check_rate_limit(
        request, "auth:register", settings.RATE_LIMIT_AUTH_PER_MINUTE, 60, "critical"
    ):
        raise HTTPException(status_code=429, detail="Too many requests")
    ...
    if settings.USE_SUPABASE:
        ...
        if settings.REFRESH_TOKENS_ENABLED and not settings.USE_SUPABASE:
...
async def login(
    ...
    if not check_rate_limit(
        request, "auth:login", settings.RATE_LIMIT_AUTH_PER_MINUTE, 60, "critical"
    ):
        log_rate_limit_exceeded(
...
async def verify_2fa(
    ...
    if not check_rate_limit(
        request, "auth:verify2fa", settings.RATE_LIMIT_AUTH_PER_MINUTE, 60, "critical"
    ):
        log_rate_limit_exceeded(
            ip_address=get_client_ip(request),
...
def _extract_refresh_token(request: Request, payload: auth_schemas.RefreshIn) -> str | None:
    if payload.refresh_token:
        return payload.refresh_token.strip()
    cookie_val = request.cookies.get(settings.REFRESH_COOKIE_NAME)
    if cookie_val:
        return cookie_val.strip()
    return None


def _load_user_claim_material(db: Session, user_id: str) -> tuple[str | None, str | None, bool]:
    user_row = (
        db.execute(
            text(
                """
    ...
    try:
        ...
        try:
            ...
            sentry_sdk.capture_message(
                f"refresh token reuse detected for user={reuse.user_id}", level="warning"
            )
        except Exception:
            pass
