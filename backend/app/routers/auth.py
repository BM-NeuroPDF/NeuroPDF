...
def google_login(request: Request, payload: auth_schemas.GoogleExchangeIn, supabase: Client = Depends(get_supabase)):
    ...
    auth_res = supabase.table("user_auth").select("user_id").eq("provider", "google").eq("provider_key", sub).execute()
    ...
    # Kullanıcı varsa bilgilerini çek
        ...
        settings_res = supabase.table("user_settings").select("eula_accepted").eq("user_id", user_id).execute()
...
def register_user(request: Request, payload: auth_schemas.RegisterIn, supabase: Client = Depends(get_supabase)):
    ...
    username_check = supabase.table("users").select("id").eq("username", payload.username).execute()
    if username_check.data:
        raise HTTPException(status_code=400, detail="Username already taken")

    # Email check (user_auth table)
    email_check = supabase.table("user_auth").select("user_id").eq("provider_key", payload.email).execute()
    if email_check.data:
...
def accept_eula(payload: auth_schemas.AcceptEulaIn, current_user: dict = Depends(get_current_user_dep), supabase: Client = Depends(get_supabase)):
    supabase.table("user_settings").update({"eula_accepted": payload.accepted}).eq("user_id", current_user["sub"]).execute()
...
def get_me(current_user: dict = Depends(get_current_user_dep), supabase: Client = Depends(get_supabase)):
    ...
    auth_res = supabase.table("user_auth").select("provider, provider_key").eq("user_id", user_id).execute()
...
def delete_account(current_user: dict = Depends(get_current_user_dep), supabase: Client = Depends(get_supabase)):
