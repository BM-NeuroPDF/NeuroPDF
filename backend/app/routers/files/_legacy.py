...
async def upload_pdf(
    ...
    try:
        ...
        if not is_guest_user:
            ...
            stats_cache_delete_keys(_files_list_cache_key(str(user_id)))
            return {
...
async def get_my_files(
    ...
    try:
        ...
        cache_key = _files_list_cache_key(str(user_id))
        cached = stats_cache_get_json(cache_key)
        if cached is not None and isinstance(cached, dict):
            log_cache(
                "my_files_redis",
        ...
        stats_cache_set_json(cache_key, payload, FILES_LIST_CACHE_TTL_SEC)
        return payload
    except HTTPException:
        raise
    except Exception as e:
